...
const {exists, tryReplaceUnit, float, quote} = require('./lib');
...
function insertMeta(jassCode, meleeMeta, evergreenMeta) {
	const {hash: meleeHash, texts: meleeTexts} = meleeMeta;
	...
	jassCode = jassCode.replace(/(set udg_MetaTextQuestCredits\[8\] = )"[^"]+"/, `$1"|cffffcc00${meleeTexts.name}|r's hash (|cff4682b4sha256|r):"`);
	jassCode = jassCode.replace(/(set udg_MetaTextQuestCredits\[9\] = )"[^"]+"/, `$1"${meleeHash}"`);
	jassCode = jassCode.replace(/(set udg_MetaTextQuestCredits\[12\] = )"[^"]+"/, `$1"${evergreenMeta.AMAIVersion}"`);
