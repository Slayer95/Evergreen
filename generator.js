...
function insertMeta(jassCode, meleeMeta, evergreenMeta) {
	...
	jassCode = jassCode.replace(/(set udg_MetaTextQuestCredits\[9\] = )"[^"]+"/, `$1"|cffffcc00${meleeTexts.name}|r's hash (|cff4682b4sha256|r):"`);
	jassCode = jassCode.replace(/(set udg_MetaTextQuestCredits\[10\] = )"[^"]+"/, `$1"${meleeHash}"`);
