...
function insertMeta(jassCode, meleeMeta, evergreenMeta) {
	const {hash: meleeHash, editorVersion: meleeEditorVersion, gameVersion, texts: meleeTexts} = meleeMeta;
	...
	const gameVersionText = `- |cffffcc00v${gameVersion}|r game mechanics port.`;
...
*/
	...
	jassCode = jassCode.replace(/(set udg_MetaTextQuestAuthor\[3\] = )"[^"]+"/, `$1"${gameVersionText}"`);
	...
	/* JASS metadata */
	jassCode = jassCode.replace(/\/\/ ([\w][^\r\n]+?)(?=\r?\n)/, `// ${meleeTexts.name} ${version}`);
	jassCode = jassCode.replace(/Generated by [^\n]+/, `Generated by ${generator}`);
	jassCode = jassCode.replace(/Date: [^\n]+/, `Date: ${date}`);
	jassCode = jassCode.replace(/Map Author: [^\n]+/, `Map Author: ${author}`);
	jassCode = jassCode.replace(/(set udg_RFTextObserverBoardTitle = "\|cffffcc00Statistics)(\|r")/, `$1 - ${meleeTexts.name} ${version} - by ${author}$2`);
