...
function insertInSection(source, header, functions) {
	...
	let fnSources = functions.map(fn => fn.hasOwnProperty(LUA_SOURCE) ? lua2jass(fn, fn[LUA_SOURCE]) : fn.source);
	fnSources.push('');
	return source.slice(0, nlIndex3 + 1) + `\r\n` + fnSources.join(`\r\n`) + source.slice(nlIndex3 + 1);
}

function insertMeta(jassCode, meleeMeta, evergreenMeta) {
	const {author, date, generator, version} = evergreenMeta;
	const questMapCreditsText = `|cffffcc00${meleeMeta.name}|r is a map made by |cffffcc00${meleeMeta.author}|r.`
	jassCode = jassCode.replace(/(set udg_MetaTextQuestCredits\[1\] = )"[^"]+"/, `$1"${questMapCreditsText}"`);
	jassCode = jassCode.replace(/\/\/ ([\w][^\r\n]+?)(?=\r?\n)/, `// ${meleeMeta.name} ${version} (for 1.26)`);
